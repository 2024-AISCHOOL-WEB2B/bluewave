...
public class PolicyDAO {
    ...
    // 정책테이블에서 ID에 있는 값 가져오기
    public List<PolicyDTO> getPoliciesByIds2(List<String> policyIds) {
        List<PolicyDTO> policies = new ArrayList<>();
        if (policyIds == null || policyIds.isEmpty()) {
            System.out.println("policyIds is null or empty");
            return policies;
        ...
        try {
            ...
            String sql = "SELECT * FROM EX_POLICY WHERE POLICY_ID IN (";
            for (int i = 0; i < policyIds.size(); i++) {
                sql += "?";
            ...
            for (int i = 0; i < policyIds.size(); i++) {
                pstmt.setString(i + 1, policyIds.get(i));
            }
